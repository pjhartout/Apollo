#!/usr/bin/env python3
# Taken from https://github.com/broadinstitute/gtex-pipeline/tree/master/gene_model
        ...
        with opener as gtf:
            for row in gtf:
                ...
                if np.mod(len(self.genes), 1000) == 0:
                    print(
                        "Parsing GTF: {0:d} genes processed\r".format(len(self.genes)),
                        end="\r",
                    )
...
def collapse_annotation(
    annot, transcript_gtf, collapsed_gtf, blacklist=set(), collapse_only=False
):
    """
        ...
        for g in annot.genes:
            if g.id in merged_coord_dict:
                for i in merged_coord_dict[g.id]:
                    ...
                    interval_trees.setdefault(g.chr, IntervalTree()).add(
                        i[0], i[1] + 1, [i, g.id]
                    )
                    ...
                    output_gtf.write(
                        "\t".join(
                            [
                                ...
                                + ' exon_id "'
                                + g.id
                                + '_{0:d}; exon_number {0:d}";'.format(k),
                            ]
                        )
                        + "\n"
                    )
